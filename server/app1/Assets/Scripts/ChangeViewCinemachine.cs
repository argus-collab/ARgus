...
public class ChangeViewCinemachine : MonoBehaviour
{
    ...
    public void HideInvisibleMiniature()
    {
    ...
    public void HideAll(bool hideFreezedHololensView = true, bool hideHololensView = true)
    {
    ...
    public void HideAllImmediately(bool hideFreezedHololensView = true, bool hideHololensView = true)
    {
        isHololensView = false;
        isVirtualView = false;
        isCustomCameraView = false;
    ...
    public void Update()
    {
        ...
        //ebug.Log("d main to holo : " + (mainCamera.transform.position - hololensCamera.transform.position).magnitude);
        ...
        if (move)// && Time.time - movingTimeStamp < movingTimeLimit)
        {
            //if ((goHololens && 
            //    (
            //        (mainCamera.transform.position - endingCameraPosition).magnitude < 0.05f
            //        //(mainCamera.transform.position - lastCameraPosition).magnitude > 0 
            //        //|| mainCamera.transform.position == startingCameraPosition
            //    ))
            //    || (!goHololens && mainCamera.transform.position != endingCameraPosition))

            if(((!goHololens && (mainCamera.transform.position - endingCameraPosition).magnitude > 0.005f)
                || (goHololens && (mainCamera.transform.position - hololensCamera.transform.position).magnitude > 0.06f))
                && Time.time - movingTimeStamp < movingTimeLimit)
            {
                MouseNavigator.isActive = false;
                ...
                if(goHololens)
                    Debug.Log("is moooooooooooving go hololens : " + (mainCamera.transform.position - hololensCamera.transform.position).magnitude);
                Debug.Log("is moooooooooooving : " + (mainCamera.transform.position - endingCameraPosition).magnitude);
        ...
        else
        {
            ...
            else if (isMouseNavigationActive && !isVirtualView && MouseNavigator.IsMoving())
            {
                ...
                else if (isKinectView)
                    TeleportVirtualCamera(KinectGO.transform.position, KinectGO.transform.rotation);
                else
                    TeleportVirtualCamera(mainCamera.transform.position, mainCamera.transform.rotation);
    ...
    public void DisplayARHeadsetViewImmediately()
    {
        Debug.Log("hololens camera view");
        cinemachine.GetMainVirtualCamera().m_Lens.FieldOfView = 33;
        virtualCamera.fieldOfView = 33;

        HideClient();

        //HideAllImmediately(true, false);
        HideAll(true, false);
        //index = 1;
        //movingTimeStamp = Time.time;
        //move = true;
